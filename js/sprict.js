...
let aiExplainBlock = document.getElementById('aiExplainBlok');
...
const guestionsong = new Audio('./music/questions-sound.mp3')
let count = 0
...
endBtn.addEventListener('click', () => {
  ...
  mainGame.classList.add('animate__animate', 'animate__backOutDown')
  setTimeout(() => {
  ...
  setTimeout(() => {
    winDiv.classList.replace('animate__fedeIn', 'animate__fadeOut')
...
btnAnswers.forEach((btnAnswers) => {
  ...
  btnAnswer.addEventListener('click', (e) => {
    let numberQustion = btnAnswer.parentElement.classList[1];
    let blokAnswer = e.target.innerText
    let blockQuestion = blokAnswer.parentElement;
    blockQuestionParentElement.classList.add('blok-event')
    correctnessAnswer(numberQustion, userActivation, blokAnswer, blockQuestionParentElement)
...
btnAnswers.forEach((item) => {
  btnAnswer.addEventListener('mousseover', () => {
    if (item.children[0]) {
      item.children[0].style.display = "none"
      item.classList.remove('colo-active')
    }
  });
});
...
helpFifty.addEventListener.play('click', function removeTwoBlock() {
  helpSound.play()
  let blocActiveQustion = getActiveBlockQuestion()
  let numbRandom = Math.floor(Math.random * blockAnstiveQusention.children[1].children.length)
  let blockChildrenAnswer = blocActiveQustion.classList[1]
  let nameQuestion = blockActiveQuestion.classList[1]
  let blockCorrectAnswer = getBlockAnswer(blockChilderenAnswwer,nameQuestion)
  blockCorrectAnswer.classList.add('fifty-active')
  let blockRandom =getBlockRandom = getBlockRandom(blockChildrenAnswer,blockChildrenAnswer,numRandom)
  removeTwoBlock(blockCorrectAnswer)
  helpFriend.classList.add('hints-hlep-spant', 'block-event')
})
