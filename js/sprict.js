...
let mainGame = document.querySelector('.game-block'),
  ...
  blockQuestions = document.querySelectorAll('.question'),
...
const correctSound = new Audio ("./music/correct-soud.mp3")
const  incorrectSoundFlag = new Audio("./music/incorrect-soud.mp3")
...
if(answer[mumberQuestion]===userAnswer){
setTimeout(()=>{
...
extraQuestion.classList.remove("question_action")
 },500 )
}
}else{
  setTimeout(() => {
...
function getRemoveClassName(){
  for (let i = 0; i< blockQuestion.length; i++){
    if(blockQuestion[i].classList.contains('question-active')){
      blockQuestion[i].classList.contains('animate__animated','animate__fadeOut')
      blockQuestion[i].classList.contains('question__active')
    }
  }
}
...
block.insertAdjacentHTML(`<div class="user-win animate__animated animate__fadeIn"><p>Ձեր հաղթանակը</p><p>"${getGarantWin()}"</p></div>`);)
}
...
function getGarantWin(){
  for (let i = 0; i< winBlock.length; i++){
    if(winBlock[i].classList.contains('win-guaranted')){
      let getUserWin = winBlock[1].innerText
...
question_2:'Գ․ Մաշկ',
question_3:'Բ․ Սենա',
question_4:'Բ․ Հունաստան',
question_5:'Բ․ Ազոտ',
question_6:'Բ․ Մարիանյան իջվածք',
question_7:'Գ․ Վինսենթ վան Գոգ',
question_8:'Բ․ Արծաթ',
question_9:'Բ․ Յուպիտեր',
question_10:'Գ․ 1989',
question_11:'Գ․ Իսահակ Նյուտոն',
question_12:'Գ․ Անտարկտիդայի բևեռային անապատ',
question_13:'Բ․ Իտալերեն',
question_14:'Բ․ Ջրածին',
question_15:'Բ․ 1215'
}